...
Managed clusters communicate with the hub cluster through a mutual connection with the {ocp-short} `KubeAPIServer` external load balancer. An internal {ocp-short} cluster certificate authority (CA) issues the default {ocp-short} `KubeAPIServer` certificate when you install {oc-short}. The {mce} automatically detects and passes the certificate to managed clusters in the `bootstrap-kubeconfig-secret` namespace.
