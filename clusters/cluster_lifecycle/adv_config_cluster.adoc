...
Managed clusters communicate with the hub cluster through a mutual connection with the {ocp-short} `KubeAPIServer` external load balancer. An internal {ocp-short} cluster certificate authority (CA) issues the default {ocp-short} `KubeAPIServer` certificate when you install {ocp-short}. The {mce} automatically detects and passes the certificate to managed clusters in the `bootstrap-kubeconfig-secret` namespace.

If your automatically detected certificate does not work, you can manually configure a strategy configuration in the `KlusterletConfig` resource. Manually configuring the a strategy allows you to control how you verify the hub cluster `KubeAPIServer` certificate.

See the examples in one of the following three strategies to learn how to manually configure a strategy:

[#config-hub-kube-api-server-detect]
=== Configuring the stragey with `UseAutoDetectedCABundle`

`UseAutoDetectedCABundle` is the default strategy. The {mce-short} automatically detects the certificate on the hub cluster and merges the certificate configured in the `trustedCABundles` list of ConfigMap references to the real CA bundles, if there are any.

The following example merges the automatically detected certificates from the hub cluster, and the certificates that you configured in the `new-ocp-ca` ConfigMap, and passes them to the managed cluster:
...
[#config-hub-kube-api-server-system]
=== Configuring the stragey with `UseSystemTruststore`

With `UseSystemTruststore`, {mce-short} does not detect any certificate and ignores the certificates configured in the `trustedCABundles`. This configuration does not pass any certificate to the managed clusters. Instead, the managed clusters use certificates in their system trusted store to verify the hub cluster API server. This applies to situations where a public CA, such as `Let's Encrypt`, issues the hub cluster certificate. See the following example that uses `UseSystemTruststore`:
...
[#config-hub-kube-api-server-custom]
=== Configuring the stragey with `UseCustomCABundles`

You can use `UseCustomCABundles` if you lknow the CA of the hub cluster API server and do not want {mce-short} to automatically detect it. For this strategy, {mce-short} passes your configured certificates from `trustedCABundles` to the managed clusters. See the following examples to learn how to use `UseCustomCABundles`:
