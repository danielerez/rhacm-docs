...
The following example merges the automatically detected certificates from the hub cluster, and the certificates that you configured in the `new-ocp-ca` ConfigMap, and adds them to the managed cluster:
...
You can use `UseCustomCABundles` if you lknow the CA of the hub cluster API server and do not want {mce-short} to automatically detect it. For this strategy, {mce-short} adds your configured certificates from `trustedCABundles` to the managed clusters. See the following examples to learn how to use `UseCustomCABundles`:
