...
Follow the link:../cleanup_reinstall.adoc#cleanup-reinstall[Cleaning up artifacts before reinstalling] procedure to prevent this problem.
...
//2.6X: 26987 -//very old, we need to consider removing
...
// 2.5, 2.4: 23730 -//very old, we need to consider removing
...
== OpenShift Container Platform cluster upgrade failed status
// 2.0.0:3442// we should document an OCP known issue. I did not add this and have been vocal against this. It is better to point to their known issues
...
== Create MultiClusterEngine button not working
//2.6:25641---//very old, we need to consider removing
