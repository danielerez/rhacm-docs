...
When you check the `ManagedClusterAddon` status, part of the configurations have an empty `spec` hash, even if the configurations exist. See the following example:
...
To resolve the issue, delete the `ManagedClusterAddon` by running the following command to reinstall and recover the `ManagedClusterAddon`. Replace `<cluster-name>` with the `ManagedClusterAddon` namespace. Replace `<addon-name>` with `ManagedClusterAddon` name:
