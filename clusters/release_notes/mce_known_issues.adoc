...
=== Cannot use host inventory for booting with the discovery image and adding hosts automatically
//2.12:ACM-14719

You cannot use a host inventory, or `InfraEnv` custom resource, for both booting with the discovery image and adding hosts automatically. If you used your old `InfraEnv` for `BareMetalHost` and want to boot the image yourself, you can work around the issue by creating a new `InfraEnv`.
