...
If you installed your {mce-short} into a customized namespace, then you cannot enable the {global-hub} search. In this case, the search operator fails to update the `console-mce-config` config map because it added the `globalSearchFeatureFlag=true`, preventing you from enabling the {global-hub} search.
