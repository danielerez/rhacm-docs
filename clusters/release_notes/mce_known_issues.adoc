...
You cannot use a host inventory, or `InfraEnv` custom resource, for both booting with the discovery image and adding hosts automatically. If you used your previous `InfraEnv` for `BareMetalHost`, and want to boot the image yourself, you can work around the issue by creating a new `InfraEnv`.
