...
When you use the `Exist` or `DoesNotExist` operators in the `Placement` resource, the application topology node details display the expressions as `#invalidExpr`. This display is wrong, and the expression is still valid and works in the `Placement` resource. To workaround around this issue, complete the following steps: 

. Open the `Placement` resource YAML. 
. Find the invalid expression and edit it. 
