...
Review the hub cluster API server certificate verification strategy to make sure that the default `UseAutoDetectedCABundle` strategy works. If you need to manually change the strategy, see xref:../cluster_lifecycle/adv_config_cluster.adoc#config-hub-kube-api-server[Configuring the hub cluster `KubeAPIServer` verification strategy].
